...
const DownloadFormatSelector = ({ 
  ...
  const generateHeifCompatibleSequence = useCallback(async (frames, width, height, frameInterval) => {
    try {
      ...
      const metadata = {
        ...
        software: 'Labubu Gallery React - HEIF Compatible Generator'
      };
  ...
  const handleFormatDownload = useCallback(async (format) => {
    ...
    try {
      ...
      let fileName = `${item.title || 'labubu-wallpaper'}`;
      ...
      if (format.id === 'gif') {
        ...
        try {
          ...
          if (retryBlob) {
            ...
            const link = document.createElement('a');
            link.href = blobUrl;
            link.download = `${item.title || 'labubu-wallpaper'}.gif`;
            link.style.display = 'none';
              ...
              video.onseeked = () => {
                ...
                canvas.toBlob((blob) => {
                  if (blob) {
                    ...
                    const link = document.createElement('a');
                    link.href = blobUrl;
                    link.download = `${item.title || 'labubu-wallpaper'}_HQ.jpg`;
                    link.style.display = 'none';
        ...
        onDownload(fallbackProxyUrl, `${item.title || 'labubu-wallpaper'}`);
      }
    }
